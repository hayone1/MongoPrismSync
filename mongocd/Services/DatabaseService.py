...
class DatabaseService(IDatabaseService):
    ...
    async def generate_indexreplication_commandasync(self, databaseConfig: DatabaseConfig) -> str | ReturnCode:
        ...
        '''
        self.logger.debug(f"GenerateIndexCreationCommandAsync Started: {databaseConfig.source_db}")
        try:
                ...
                get_indices_tasks = [
                    tgrp.create_task(
                        self.clients[databaseConfig.name].ShCommandAsync(
                                f'JSON.stringify({{"collName": "{collection_property.name}",\
    ...
    def generate_createcollection_command(self, db_name: str, collection_name: str) -> str:
        self.logger.debug(f"adding create_collection_command for {collection_name}")
    ...
    async def generate_syncscripts_async(self) -> ReturnCode:
        ...
        with progress:
            print(f"Starting: build weave queries")
            for databaseConfig in self.databaseConfigs:
                if databaseConfig.skip == True:
                    print(f"[yellow]-[/yellow] Skipping: {databaseConfig.name}")
                    continue
                db_weave_task = progress.add_task(description=f"building weave queries for dbConfig: {databaseConfig.name}", total=1)
                ...
                # using data from source db's collections, add commands to add/update data in destination db's collection
                formDocumentsDataTasks: list[Task[ReturnCode]] = []
                async with progress, asyncio.TaskGroup() as tgrp:
                    for collection_property in collection_properties:
                        form_data_task = self.collection_service \
                            .form_documentsdata_async(databaseConfig, 
                                collection_property, database_folder)
                        form_data_task.add_done_callback()
                        formDocumentsDataTasks.append(form_data_task)
                ...
                # print(f"Completed: build weave queries for dbConfig: {databaseConfig.name}")
